...
```py
# Don't do this
temp = get_house_price_in_usd(house_square_foot, house_room_count)
final_value = temp * usd_to_aud_conversion_rate
# Do this instead
house_price_in_usd = get_house_price_in_usd(house_square_foot,
                                            house_room_count)
house_price_in_aud = house_price_in_usd * usd_to_aud_conversion_rate
```

* If you are using abbreviations like `usd, aud, mph, kwh` make sure you establish these ahead of time. Agree with the rest of your team on common abbreviations and write them down. Then, in code review, make sure to enforce these written standards.
...
$$ \text{house price} = \text{price per room} \times \text{rooms} + \text{price per floor squared} \times floors^2 + \text{expected mean house price}$$
