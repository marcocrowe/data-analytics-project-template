...
from population_planning.dataframe_labels import AGE, POPULATION, SEX, YEAR
...
def build_interactive_population_pyramid_chart(
    ...
    x_male = altair.X(
        f"sum({population_field}):Q",
        scale=scale,
        sort=sort_order,
        title="Male Population (1000)",
    )
    x_female = altair.X(
        f"sum({population_field}):Q", scale=scale, title="Female Population (1000)"
    )
    y = altair.Y(f"{age_field}:O", axis=None, sort=sort_order)

    def sex_tree_lambda(value, title, x_axis):
        return (
            trunk.transform_filter(altair.datum[sex_field] == value)
    ...
    y_trunk = (
        trunk.encode(text=altair.Text(f"{age_field}:O"), y=y)
        .mark_text()
        .properties(width=20)
    )
