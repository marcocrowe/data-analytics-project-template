...
github.display_jupyter_notebook_data_sources(
    [artifact_manager.get_population_eda_filepath()])
artifact_manager.is_remote = False


# ### Graph

def build_interactive_population_pyramid_chart(
        population_dataframe: DataFrame,
        age_field: str = "Age",
    ...
    year_bind_range = altair.binding_range(min=year_min,
                                           max=year_max,
                                           step=1,
                                           name="Select Year")
    ...
    tree_color_scale = altair.Scale(domain=[male_value, female_value],
    ...
    x_male = altair.X(f"sum({population_field}):Q",
                      scale=scale,
                      sort=sort_order,
                      title="Male Population (1000)")
    x_female = altair.X(f"sum({population_field}):Q",
                        scale=scale,
                        title="Female Population (1000)")
    y = altair.Y(f"{age_field}:O", axis=None, sort=sort_order)

    def sex_tree_lambda(value, title, x_axis):
        return trunk.transform_filter(altair.datum[sex_field] == value).encode(
            color=color, x=x_axis, y=y).mark_bar().properties(title=title)

    female_tree = sex_tree_lambda("Female", "female", x_female)
    male_tree = sex_tree_lambda("Male", "male", x_male)

    y_trunk = trunk.encode(text=altair.Text(f"{age_field}:O"),
                           y=y).mark_text().properties(width=20)
...
population_dataframe = pandas.read_csv(
    artifact_manager.get_population_eda_filepath())
...
strange_values_dataframe = strange_values_dataframe[(
    (population_dataframe[AGE] == "01-04") &
