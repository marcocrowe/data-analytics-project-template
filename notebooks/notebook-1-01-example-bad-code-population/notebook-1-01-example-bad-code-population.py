...
# print(github.create_jupyter_notebook_header("marcocrowe", "data-analytics-project-template", "notebooks/notebook-1-01-example-bad-code-population.ipynb", "master"))
# -->
...
# %% [markdown]
# A bad example of code and structure.
...
# %%
pd.Series(
    df_PP["population"].where(df_PP["Sex"] == "Both sexes").unique()
).sort_values()
...
# %%
male_tree = (
    trunk.transform_filter(alt.datum.gender == "Male")
    .encode(
        y=alt.Y("Age:O", axis=None, sort=alt.SortOrder("descending")),
        x=alt.X(
            "sum(population):Q",
            scale=alt.Scale(domain=[0, 300.0]),
...
# %%
female_tree = (
    trunk.transform_filter(alt.datum.gender == "Female")
    .encode(
        y=alt.Y("Age:O", axis=None, sort=alt.SortOrder("descending")),
        x=alt.X(
            "sum(population):Q",
